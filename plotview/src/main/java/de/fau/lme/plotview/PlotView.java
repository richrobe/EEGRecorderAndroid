...
public class PlotView extends View
{
    ...
    protected static final int AXIS_PADDING = 10;
    /** length of "pins" on the axis, indicating value text */
    protected static final int AXIS_PIN_LENGTH = 5;
    ...
    protected float dpScale = 1f;
    protected AxisPaint valueAxisPaint = null;
    protected AxisPaint domainAxisPaint = null;
        ...
        public void onLongPress( MotionEvent ev )
        {
            ...
            if (m_surface.masterPlot != null)
            {
                ...
                intBuilder.setPositiveButton( "Go", new DialogInterface.OnClickListener() {
                    ...
                    {
                        scrollTo( Float.parseFloat( input.getText().toString() ) );
    ...
    protected class PlotSurface
    {
         ...
         *
         * @param noFancyStuff
         *            Don't you dare playing games with me, boy!
         */
    ...
    private void initPlotView( Context context )
    {
        ...
        titlePaint = Plot.generatePlotPaint( 1f, 255, 75, 75, 75 );
        titlePaint.setTextAlign( Align.CENTER );
        titlePaint.setStyle( Paint.Style.FILL_AND_STROKE );
        titlePaint.setFakeBoldText( true );
        titlePaint.setTextSize( PlotView.DEFAULT_TEXT_SIZE * dpScale * 1.3f );


        Paint textPaint = Plot.generatePlotPaint( 1f, 255, 125, 125, 125 );
        textPaint.setTextAlign( Align.CENTER );
        textPaint.setStyle( Paint.Style.FILL_AND_STROKE );
        textPaint.setTextSize( PlotView.DEFAULT_TEXT_SIZE * dpScale );

        Paint textPaintLeft = Plot.generatePlotPaint( 1f, 255, 125, 125, 125 );
        textPaintLeft.setTextAlign( Align.CENTER );
        textPaintLeft.setStyle( Paint.Style.FILL_AND_STROKE );
        textPaintLeft.setTextSize( PlotView.DEFAULT_TEXT_SIZE * dpScale );

        valueAxisPaint = new AxisPaint( Plot.generatePlotPaint( 2f, 255, 125, 125, 125 ), textPaintLeft );
        domainAxisPaint = new AxisPaint( Plot.generatePlotPaint( 2f, 255, 125, 125, 125 ), textPaint );
    }
    ...
    {
        if ( !usePost && hasFlag( PlotView.Flags.USE_THREAD_SAFE_UI_CALLS ) )
            usePost = true;
    ...
    {
        m_surface.xTrans = (float) (-m_surface.masterPlot.values.num + position + m_surface.viewWidth * 0.5);
        requestRedraw( true );
    }

    public void scrollBack( float absoluteDistanceToHead )
    {
        m_surface.xTrans = -absoluteDistanceToHead + m_surface.viewWidth * 0.5f;
        requestRedraw( true );
    }
    ...
    private void checkPaint()
    {
        if (m_defaultPaint == null)
        {
            ...
            //TypedValue.applyDimension( TypedValue.COMPLEX_UNIT_DIP, DEFAULT_TEXT_SIZE, getResources().getDisplayMetrics() );
            m_defaultPaint.setTextSize( DEFAULT_TEXT_SIZE * dpScale );
        ...
        if (s_mapPaint == null)
        {
            ...
            s_mapPaint.setARGB( 128, 150, 150, 150 );
    ...
    transient Float _t_axisHeight = null;
    protected float getAxisHeight()
    {
        if (_t_axisHeight != null)
            return _t_axisHeight;

        Rect rect = new Rect();
        Rect rect2 = new Rect();
        domainAxisPaint.title.getTextBounds( "A", 0, 1, rect );
        valueAxisPaint.title.getTextBounds( "A", 0, 1, rect2 );
        _t_axisHeight = new Float( Math.max( rect.height(), rect2.height() ) );
        return _t_axisHeight;
    }
    ...
    protected void onDraw( Canvas canvas )
    {
        ...
        m_surface.plotView = this;

        m_surface.viewHeight = getHeight();
        m_surface.viewWidth = getWidth();

        // fill plot viewport rect
        m_surface.width = (int) (m_surface.viewWidth - getAxisHeight() - this.getPaddingRight() - this.getPaddingLeft());
        m_surface.height = (int) (m_surface.viewHeight - getAxisHeight() - AXIS_PADDING - this.getPaddingTop() - this.getPaddingBottom());
        ...
        if (m_plotFlags.contains( Flags.DRAW_AXES ))
        {
            ...
            titlePaint.getTextBounds( m_surface.masterPlot.plotTitle, 0, m_surface.masterPlot.plotTitle.length(), bounds );
            canvas.drawText( m_surface.masterPlot.plotTitle, m_surface.viewWidth * 0.5f, 10 + bounds.height(), titlePaint );
            ...
            m_surface.masterPlot.drawAxis( canvas, m_surface, m_plotFlags.contains( Flags.DRAW_GRID ),
                    m_plotFlags.contains( Flags.DRAW_MAP ), domainAxisPaint, valueAxisPaint );
        }
        ...
        canvas.translate( getAxisHeight() + this.getPaddingLeft() + AXIS_PADDING, getAxisHeight() + this.getPaddingBottom() + AXIS_PADDING );
